...
{
    internal class MyCarouselPageAdapter : PagerAdapter, ViewPager.IOnPageChangeListener
    {
        ...
        public MyCarouselPageAdapter(ViewPager pager, CarouselPage page, Context context)
        {
            _pager = pager;
            _page = page;
            _context = context;
        ...
        {
            var holder = (MyObjectJavaBox<Tuple<ViewGroup, Page, int>>)p2;
        ...
        public override int GetItemPosition(Object item)
        {
            ...
            var holder = (MyObjectJavaBox<Tuple<ViewGroup, Page, int>>)item;
        ...
        public override Object InstantiateItem(ViewGroup container, int position)
        {
            ...
            if (Platform.GetRenderer(child) == null)
                Platform.SetRenderer(child, Platform.CreateRendererWithContext(child, _context));

            IVisualElementRenderer renderer = Platform.GetRenderer(child);
            renderer.View.RemoveFromParent();

            ViewGroup frame = new MyPageContainer(_context, renderer);

            container.AddView(frame);

            return new MyObjectJavaBox<Tuple<ViewGroup, Page, int>>(new Tuple<ViewGroup, Page, int>(frame, child, position));
        }

        public override bool IsViewFromObject(global::Android.Views.View p0, Object p1)
        {
            var holder = (MyObjectJavaBox<Tuple<ViewGroup, Page, int>>)p1;
